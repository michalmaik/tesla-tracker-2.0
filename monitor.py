...
# Pliki stanu — nadpisywane dynamicznie w main() na podstawie MONITOR_MODE
...
def get_exchange_rates():
    try:
        resp = requests.get(
            "https://api.frankfurter.app/latest?from=PLN&to=EUR,SEK,DKK,CZK,HUF,RON,BGN",
            timeout=10
        )
        data = resp.json()
...
def make_car_id(country, url, title, year):
    """Stabilne ID oparte na slugu z URL (zawiera VIN)."""
    slug = url.rstrip("/").split("/")[-1]
    if slug and not slug.isdigit() and len(slug) > 4:
        return f"{country}-{slug}"
    # fallback gdy URL nie zawiera VIN
...
def fetch_country(session, country_cfg, rates):
    ...
    form_data = {
        ...
        "minyear": str(MIN_YEAR_CPO), "maxyear": str(MAX_YEAR),
        "minrange": "0", "offset": "0",
    }
...
def build_new_car_embed(car_id, car, price, rates):
    ...
    lines = [
        ...
        f"📅 {year}  |  🛣️ {mileage}",
        f"🔗 [Zobacz na ev-inventory.com]({url})",
    ]
    return {
...
def build_price_drop_embed(car_id, car, old_price, new_price, rates, history):
    ...
    history_str = format_price_history(history.get(car_id, []))
    if history_str:
        lines.append(f"\n📊 Historia:\n{history_str}")
...
def build_daily_summary_embed(current_cars, rates):
    ...
    by_country = {}
    for car in current_cars.values():
        by_country.setdefault(car.get("country", "?"), []).append(car)
...
def build_test_embed(current_cars, rates):
    lines = []
    for country in sorted(set(car.get("country", "?") for car in current_cars.values())):
        flag     = COUNTRY_FLAGS.get(country, "🇪🇺")
        currency = get_currency_for_country(country)
        for car in sorted(
            [c for c in current_cars.values() if c.get("country") == country],
...
def main():
    ...
    if is_manual:
        send_discord([build_test_embed(current_cars, rates)])
